...
#pragma clang diagnostic pop
    ...
    int iResult = shutdown(m_pImpl->m_connectSocket, SD_SEND);
    if(iResult == SOCKET_ERROR) {
        //printf("shutdown failed with error: %d\n", WSAGetLastError());
        closesocket(m_pImpl->m_connectSocket);
        WSACleanup();
...
void grcore::util::TcpClient::init() {
    ...
    // Initialize Winsock
    iResult = WSAStartup(MAKEWORD(2, 2), &m_pImpl->m_wsaData);
    if(iResult != 0) {
    ...
    for(ptr = result; ptr != NULL; ptr = ptr->ai_next) {
        ...
        m_pImpl->m_connectSocket = socket(ptr->ai_family, ptr->ai_socktype,
                ptr->ai_protocol);
        if(m_pImpl->m_connectSocket == INVALID_SOCKET) {
            WSACleanup();
            throw std::runtime_error(fmt::format("socket failed with error: {}", WSAGetLastError()));
        }

        // Connect to server.
        iResult = connect(m_pImpl->m_connectSocket, ptr->ai_addr, static_cast<int>(ptr->ai_addrlen));
        if(iResult == SOCKET_ERROR) {
            closesocket(m_pImpl->m_connectSocket);
            m_pImpl->m_connectSocket = INVALID_SOCKET;
            continue;
        }
